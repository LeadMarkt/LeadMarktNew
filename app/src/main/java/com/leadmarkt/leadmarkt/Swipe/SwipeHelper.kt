package com.leadmarkt.leadmarkt.Swipe
    ...
    ItemTouchHelper.SimpleCallback(0,ItemTouchHelper.LEFT)
{
    ...
    val buttonBuffer: MutableMap<Int,MutableList<SwipeButton>>
    ...
    private fun drawButton(c: Canvas, itemView: View, buffer: MutableList<SwipeButton>, translationX: Float) {
        var right = itemView.right.toFloat()
        val dButtonWidth = -1*translationX/buffer.size
        for (button in buffer){
            val left = right - dButtonWidth
            button.onDraw(c, RectF(left,itemView.top.toFloat(),right,itemView.bottom.toFloat()),pos = 0)
            right = left
        }
    }
}